...
public class Simulator implements WorldListener {
	...
	//private int casualties; // dah gedeed
	...
	private void loadUnits(String path) throws Exception {
		...
		while (line != null) {
			...
			switch (info[0]) {
			case "AMB": {
				Ambulance a = new Ambulance(id, world[0][0], steps, this); //worldAddress as simulator
	...
	public int calculateCasualties() {
		...
		for (int i = 0; i < citizens.size(); i++) {
			if (citizens.get(i).getState() == CitizenState.DECEASED) {
				count++;
				System.out.println("Casualtiessss  -----------------:\n" + citizens.get(i));
			}
		}
		return count;
	}
	...
	public String toString() {
		return "Current Cycle is: " + currentCycle;
	}
